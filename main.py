...
# Dict key is the Github org name to sync from,
# value is the Gitlab org/project name to sync to.
SYNC_MAPPING = {
    'JawboneHealth': {'name': 'JHH', 'type': 'org'},
    'richardARPANET': {'name': 'richard', 'type': 'user'},
    'odwyersoftware': {'name': 'odwyersoftware', 'type': 'org'},
    'VeemsHQ': {'name': 'Veems', 'type': 'org'},
}
...
def main():
    for result in ThreadPool(len(SYNC_MAPPING)).imap_unordered(
        _process_org_or_user, SYNC_MAPPING
    ):
        print(result)
...
def _push_repo_to_gitlab(repo):
    ...
    repo_owner_name = SYNC_MAPPING[org_name or owner_name]['name']
    owner_type = SYNC_MAPPING[org_name or owner_name]['type']
    ...
    return group_exists, user_exists, repo_exists
